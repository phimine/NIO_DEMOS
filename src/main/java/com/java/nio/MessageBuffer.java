...
public class MessageBuffer {
	...
	byte[] smallMessageBuffer = new byte[5 * 4 * KB]; // 1024 x 4KB messages = 4MB.
...
//    	System.out.println("message buffer # get message!");
        int nextFreeSmallBlock = this.smallMessageBufferFreeBlocks.take();
        System.out.println("========================free small block : " + nextFreeSmallBlock);
        ...
        System.out.println("========================get message --  capacity : " + message.capacity + ", offset : " + message.offset  + ", length : " + message.length);
        return message;
    }
	...
	public void freeMessageBlock(Message message) {
		if (message.capacity == CAPACITY_SMALL) {
			this.smallMessageBufferFreeBlocks.put(message.offset);
		} else if (message.capacity == CAPACITY_MEDIUM) {
			this.mediumMessageBufferFreeBlocks.put(message.offset);
		} else {
			this.largeMessageBufferFreeBlocks.put(message.offset);
		}
	}
