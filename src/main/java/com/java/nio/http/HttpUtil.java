...
public class HttpUtil {
	 ...
	 * @param endIndex
	 * @param httpHeaders
	 * @return the index of body end, if this is not full request return -1
	 * @author FuMingming
	 */
	...
	/**
	 * find the value of Content-Length and set it to httpHeader
	 * 
	 * @param src
	 * @param startIndex
	 * @param endIndex
	...
	/**
	 * find out the index of next specific byte in the scope of the byte array
	 * 
	 * @param src
	 *            byte array
	 * @param startIndex
	...
	/**
	 * find out the index of next line break[\n\r] in specific scope of the byte
	 * array
	 * 
	 * @param src
	 * @param startIndex
	...
	public static void resolveHttpMethod(byte[] src, int startIndex, HttpHeaders httpHeaders) {
		if (matches(src, startIndex, GET)) {
			httpHeaders.httpMethod = HttpMethodEnum.HTTP_METHOD_GET;
			return;
		}
		if (matches(src, startIndex, POST)) {
			httpHeaders.httpMethod = HttpMethodEnum.HTTP_METHOD_POST;
			return;
		}
		if (matches(src, startIndex, PUT)) {
			httpHeaders.httpMethod = HttpMethodEnum.HTTP_METHOD_PUT;
			return;
		}
		if (matches(src, startIndex, HEAD)) {
			httpHeaders.httpMethod = HttpMethodEnum.HTTP_METHOD_HEAD;
			return;
		}
		if (matches(src, startIndex, DELETE)) {
			httpHeaders.httpMethod = HttpMethodEnum.HTTP_METHOD_DELETE;
			return;
		}
	}

	/**
	 * determine if the byte array contains the specific value
	 * 
	 * @param src
	 * @param offset
	 * @param value
